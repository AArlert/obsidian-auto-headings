...
> 工作流的可视化说明（面向人类读者）：[`doc/workflow.html`](./doc/workflow.html)。
...
**接手第一条命令**：有 `npm run docs` 的 Addon 先跑 **`npm run docs -- --handover`**——一次打印
「status 首行总览 + log.md 最新块 + testplan 待办摘要」，代替手动读下列三个文件。没有该脚本的
Addon 才按此顺序读（不要通读代码或全部日志）：
...
| `<addon>/doc/status.jsonl` | 状态索引（首行总览 + 最新 N 条周期概括，倒序） | 每周期更新，收尾脚本滚动 |
| `<addon>/doc/status-archive.jsonl` | 由 `status.jsonl` 滚动出去的历史概括行（倒序） | 归档脚本自动维护，**平时不读** |
...
其他专项常驻文档（如 `competitive.md` 竞品分析）不进上表，由 `spec.md` 相应章节给一行链接引用。

> **文档维护脚本化**：有 `scripts/docs.mjs`（`npm run docs`）的 Addon，由它负责机械整理——
> 归档 `log.md` 旧周期块、滚动 `status.jsonl`、打印 `testplan` 摘要、校验 `status.jsonl` 与
> 「目录结构约定」常青块（与磁盘双向比对，防漂移）。Agent 只写语义内容（新周期块、状态概括），
> **机械的挪动交给脚本**。没有该脚本的 Addon 才手动维护。
...
> **上架后策略**（对已发布到用户的 Addon）：改为**仅行为 / 产物变化才 bump** manifest 版本——纯文档
> 改动只记 `log.md`，避免向线上用户推送无内容更新（obsidian-auto-headings 自 1.0.0 起适用，见其 spec §5 M7）。
...
| Addon | 类型 | 状态 | 详情 |
|-------|------|------|------|
| [`obsidian-auto-headings/`](./obsidian-auto-headings/) | Obsidian 插件：按模板自动为 Markdown 标题编号 | 开发中（M7 上架冲刺） | [`README`](./obsidian-auto-headings/README.md) · [`spec`](./obsidian-auto-headings/doc/spec.md) · [`log`](./obsidian-auto-headings/doc/log.md) · [`status`](./obsidian-auto-headings/doc/status.jsonl) |
...
**pre-commit 文档守卫**（`.githooks/pre-commit`）：提交时对每个「自带 `scripts/docs.mjs` 且本次有暂存改动」的 Addon 跑 `node scripts/docs.mjs --check`——`log.md` 周期块超上限（忘归档）、`status.jsonl` 概括行超上限（忘滚动）、或「目录结构约定」常青块与磁盘不一致（新增/拆分文件忘回填），任一命中即**拦下提交**。修复：在该 Addon 跑 `npm run docs`（目录树漂移需手动修缮 log.md 该块）后 `git add` 重提；确需跳过用 `git commit --no-verify`。本地克隆首次需手动 `git config core.hooksPath .githooks`（远程会话由 SessionStart 自动设）。

**CI**（`.github/workflows/ci.yml`）：push 到 `master` 与 PR 时，对每个带工具链的 Addon 跑完整质量门槛（文档守卫 + test + lint + format:check + build）——pre-commit 可被 `--no-verify` 跳过，CI 是机器兜底。新增带工具链的 Addon 时照抄一个 job。
