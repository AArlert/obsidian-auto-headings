...
| 大范围只读检索：翻 log-archive、跨多个文件汇总线索           | `repo-scout`（haiku）     | 结论 + file:line + 最小摘录，≤20 行 |
| 改动面大的机械编辑：批量重命名、成批 i18n 文案、全仓格式修复 | `mech-editor`（haiku）    | 改动文件清单 + 自检结果，≤15 行     |
| 边界清晰、可独立并行的编码任务                               | `feature-coder`（sonnet） | 摘要 + 触碰文件 + 门槛结果，≤25 行  |

**绿灯路径不派**：质量门槛直接跑 `npm run check`（输出已压缩为只报问题）；读单个 spec 节、单个
testplan 场景组、grep 一个符号，主模型直接做。
...
**每个开发周期结束必须**在 `doc/log.md` 顶部追加周期块：日期 / 交接人（分支名）、做了什么、没做什么、
下一步、验证方式；写完跑 `npm run docs`，脚本把超出保留数的旧块滚进 `log-archive.md`。

> `doc/log.md` 顶部导语可能有本仓库专属规则，优先级高于本文件，必须先读。
...
见 [`doc/log.md`](./doc/log.md) 最新周期块（`npm run docs -- --handover` 会打印）。不在本文件复述，
避免多处维护、彼此漂移。
