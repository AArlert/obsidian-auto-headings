...
## 2026-08-09 真机回归：新敲的标题不编号——彻底换掉「整行冻结」（1.0.24）

> **接手者注意**：本块记录的是**我上一轮修得不对**、用户真机又打回来的一次。教训在最后一节，
> 比改动本身值钱。
...
根因是**上一轮（1.0.23）我给的修法覆盖不全**。1.0.23 的判据是「光标行的标题层级相对
...
**新敲出一个标题恰好会让标题数量变化**，于是走保守分支、整行冻结——1.0.23 只挡住了「改层级」
那一种症状，挡不住「新增标题」这一种。
...
`stripPrefix` 的 `\s+$` 把刚敲的空格吃掉）。1.0.24 把手段本身换掉：
...
`tsc`/`lint`/`format:check` 全绿。

### 教训（比改动本身重要）★

1. **上一轮我把用户的反馈修窄了。** 用户 1.0.23 那次说的是「改层级后等不到编号」，我就只针对
...
- **没有引入光标 / 选区监听**（CM6 `updateListener` 一类）。1.0.24 是绕开这条路解决的；spec
  Roadmap 里 hobeedzc 那条「脏标题行集合 + 离开该行才重排」仍未做，且已补注说明它依赖的正是这次
  绕开的事件源，真要做得重新估成本。
- **没动 1.0.23 的清理确认框相关代码**（勾选 / diff / 搜索），本次改动与它无交集。

### 下一步

1. **待用户真机复验**：新敲标题当轮编号、改层级当轮生效、行尾空格不被吞三条一起看。
2. 沿用的手验清单：P12 / E36 / O11① / O5f / H12 / H9 / Dataview；J18 的 DOM 交互。
3. 发版前照例 `git ls-remote --tags origin` 现场核对。
